...
class _HospitalBookingScreenState extends State<HospitalBookingScreen> {
  ...
  String _selectedServiceType = 'normal'; // 'normal' | 'vip'
  ...
  Future<void> _loadBookedSlotsForSelectedDay() async {
    ...
    try {
      final snap = await FirebaseFirestore.instance
          ...
          .where('bookingType', isEqualTo: 'hospital')
          .where('hospitalId', isEqualTo: widget.hospital.id)
      ...
      debugPrint('Lỗi load booked slots hospital: $e');
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text('Đặt lịch: ${widget.hospital.name}'),
        elevation: 1,
              ...
              child: Column(
                children: [
                  ...
                  _buildSection(
                    ...
                    child: _buildPatientSection(),
                  ),
                  _buildSection(
                    title: 'Hình thức khám',
                    child: _buildServiceTypeSelector(),
                  ),
                  _buildSection(
    ...
    return Column(
      ...
      children: [
        ...
        Wrap(
          ...
          children: [
            ChoiceChip(
              label: const Text('Khám thường'),
              selected: _selectedServiceType == 'normal',
              onSelected: (_) {
                setState(() {
                  _selectedServiceType = 'normal';
                });
              },
            ),
            ChoiceChip(
              label: const Text('Khám VIP'),
              selected: _selectedServiceType == 'vip',
              onSelected: (_) {
                setState(() {
                  _selectedServiceType = 'vip';
                });
              },
            ),
          ],
      ...
      children: [
        if (_morningSlots.isNotEmpty) ...[
          ...
          Wrap(
            ...
            runSpacing: 8.0,
            children:
                _morningSlots.map((t) => _buildTimeSlotChip(t)).toList(),
          ),
          const SizedBox(height: 16),
        ],
        if (_afternoonSlots.isNotEmpty) ...[
          ...
          Wrap(
            ...
            runSpacing: 8.0,
            children:
                _afternoonSlots.map((t) => _buildTimeSlotChip(t)).toList(),
          ),
        ],
      ],
    );
  ...
  Widget _buildOptionalInfo() {
    return InkWell(
      onTap: () {
        // TODO: mở màn hình nhập info thêm nếu cần
      },
      child: Row(
        mainAxisAlignment: MainAxisAlignment.spaceBetween,
        children: [
